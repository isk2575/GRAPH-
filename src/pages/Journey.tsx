...
import { suggestedLetterType } from "../lib/disputeTargets";
...
export default function Journey() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchCoachPlan(profile), fetchProgress()]).then(
      ([planRes, prog]) => {
        ...
        setActiveHabits(prog.activeHabits);
        setDoneStops(prog.doneStops);
        setLoading(false);
      }
    );
  ...
  const toggleHabit = (id: string) => {
    const next = activeHabits.includes(id)
      ? activeHabits.filter((x) => x !== id)
      : [...activeHabits, id];
    setActiveHabits(next);
    saveProgress({ activeHabits: next, doneStops });
  };

  const stopDone = (id: string) => doneStops.includes(id);
  const toggleStop = (id: string) => {
    const next = doneStops.includes(id)
      ? doneStops.filter((x) => x !== id)
      : [...doneStops, id];
    setDoneStops(next);
    saveProgress({ activeHabits, doneStops: next });
  };
  ...
  const selectedLetterType = selected
    ? suggestedLetterType(selected.factor, selected.title)
    : null;
      ...
      <div className="mx-auto max-w-2xl px-6 pb-32 pt-8">
        {loading ? (
          <p className="text-center text-white/55">
            Loading your {profile.goal.timeframeMonths}-month plan…
          </p>
        ) : (
          <>
            ...
            <div className="mt-8 flex flex-wrap items-baseline gap-x-2 px-1">
              <h2 className="text-sm font-black uppercase tracking-[0.14em]">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        {isMilestone && isFree && !done && (
                          <span className="absolute -right-1 -top-1 rounded-full border border-emerald-500/40 bg-emerald-500/15 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider text-emerald-400">
                            Free
                          </span>
                        )}
            ...
            <div className="mt-10 flex items-start gap-2 rounded-2xl border border-white/[0.08] bg-[#111113] p-4 text-xs leading-relaxed text-white/45">
              <Info size={14} className="mt-0.5 shrink-0" />
              <p>
                Smaller stops are checkpoints — months where the work is simply
                sustaining your habits while earlier changes re-report. Steps
                marked <span className="font-bold text-emerald-400">Free</span>{" "}
                cost nothing to do. Momentum tracks your effort; your readiness
                score reflects your real financial data and only moves when your
                numbers do.
              </p>
            </div>
          </>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
          ...
          <div className="relative w-full max-w-md rounded-t-[2rem] border border-white/[0.08] bg-[#141416] p-6 sm:rounded-[2rem]">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {selected.cost === "free" && (
                <span className="rounded-full border border-emerald-500/40 bg-emerald-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-400">
                  Free
                </span>
              )}
            </div>
            ...
            {selectedLetterType && (
              <button
                onClick={() => navigate(`/disputes?type=${selectedLetterType}`)}
                className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#E50914]/40 bg-[#E50914]/[0.08] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#E50914]/[0.14]"
              >
                <FileText size={15} />
                Draft this letter
              </button>
